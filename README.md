...
Раздел **📋 Запросы на вознаграждение** — кликабельный список заявок. В карточке доступны кнопки **✅ Принять** и **❌ Отклонить**.
Раздел **🗂️ История вознаграждений** показывает все принятые или отклонённые запросы.
В админ-панели доступны кнопки **Назад** и **Отмена**, чтобы прервать редактирование и вернуться к меню.

## Запрос вознаграждения
На экране `/start` есть кнопка **🎁 Отправить запрос на вознаграждение**. Пользователь вводит:
1. банк;
2. телефон;
3. имя;
4. фамилию.
Повторный запрос запрещён, пока предыдущий имеет статус `pending`.
